...
func BuildGoogleURLs(searchTerm string, countryCode string, langCode string, pages int, count int) ([]string, error) {
	toScrape := []string{}
	...
	if googleBase, found := googleDomains[countryCode]; found {
		for i := 0; i < pages; i++ {
			start := count * i
			scrapeUrl := fmt.Sprintf("%s%s&num=%d&hl=%s&start=%d&filter=0", googleBase, searchTerm, count, langCode, start)
			toScrape = append(toScrape, scrapeUrl)
		}
	} else {
		err := fmt.Errorf("Country code %s not available", countryCode)
		return nil, err
	}
	return toScrape, nil
}
...
func ScrapeClientRequest(searchUrl string, proxyString interface{}) (*http.Response, error) {
	...
	if res.StatusCode != 200 {
		err := fmt.Errorf("Received a non 200 status code")
		return nil, err
	}
	if err != nil {
		return nil, err
...
func GoogleScrape(searchTerm string, countryCode string, langCode string, proxyString interface{}, pages int, count int, backoff int) ([]SearchResult, error) {
	results := []SearchResult{}
	resCounter := 0
	googlePages, err := BuildGoogleURLs(searchTerm, countryCode, langCode, pages, count)
	if err != nil {
		return nil, err
	}
	for _, page := range googlePages {
		...
		data, err := GoogleResultParsing(res, resCounter)
		if err != nil {
			return nil, err
		}
		resCounter += len(data)
...
func main() {
	response, err := GoogleScrape("Noragami", "com", "en", nil, 1, 30, 10) //30 is the number of results that we want
	if err == nil {
